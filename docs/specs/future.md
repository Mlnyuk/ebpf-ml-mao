...
이 문서는 현재 클러스터 구조([spec.md](./spec.md))를 바탕으로, 나중에 eBPF 수집과 분석 파이프라인이 실제 운영 경로에 완전히 연결됐을 때 아키텍처가 어떻게 바뀌는지 정리한 미래 구조 문서다.
...
현재 [spec.md](./spec.md) 기준 실제 운영 스택은 이미 충분히 갖춰져 있다. eBPF가 본격적으로 들어가면 아래 계층이 추가되거나 비중이 커진다.
